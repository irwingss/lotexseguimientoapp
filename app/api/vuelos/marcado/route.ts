...
// - lat?, lon? (optional, capture not wired yet; geometry sent as null for now)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.rpc('rpc_set_vuelo_marcado', {
      p_vuelo_id: vuelo_id,
      p_status: status,
      p_motivo: motivo,
      p_captura_geom: null, // geometry capture optional, not provided by this endpoint yet
